...
import { Button, IconCheckOutline16, IconChevronDownOutline14, IconCopyOutline16, IconLightOutline16, IconSparkle16, Switch, Tag } from '@deepseek-ai/dsh-client-ui-primitives'
import type { InjectFace, PropsLocale, PropsRuntime } from '@deepseek-ai/dsh-client-ui-slots'
import type { ModelCatalog } from '@deepseek-ai/dsh-api-remotes/client'
import type { RemoteResult } from '@deepseek-ai/dsh-typert-protocol'
import type {
  DescribeWorkflowData, DescribeWorkflowRequest,
  FetchWorkflowData, FetchWorkflowRequest, JsonValue,
  NodeParamOverride, RunTestData, RunTestRequest,
  ValidateWorkflowData, ValidateWorkflowRequest, WorkflowDefinition,
} from '../types.ts'
...
export interface RunningHubCardRemote {
  fetchWorkflow: (request: FetchWorkflowRequest) => Promise<RemoteResult<FetchWorkflowData>>
  testConnection: () => Promise<RemoteResult<boolean>>
  validateWorkflow: (request: ValidateWorkflowRequest) => Promise<RemoteResult<ValidateWorkflowData>>
  runTest: (request: RunTestRequest) => Promise<RemoteResult<RunTestData>>
  describeWorkflow: (request: DescribeWorkflowRequest) => Promise<RemoteResult<DescribeWorkflowData>>
...
export type RunningHubCardProps =
...
export function RunningHubCard(props: RunningHubCardProps) {
  ...
  const [validations, setValidations] = useState<Record<string, ValidateWorkflowData>>({})
  ...
  const describeProvider = slashIndex > 0 ? describeModelText.slice(0, slashIndex) : ''
  const describeModelId = slashIndex > 0 ? describeModelText.slice(slashIndex + 1) : ''
  ...
  const describeSelectionLabel = (() => {
    if (describeProvider === '') {
      if (catalog !== null) return t('describeModelDefaultOf', { model: `${catalog.default.provider}/${catalog.default.model}` })
      return t('describeModelDefault')
    }
    const group = catalog?.groups.find(candidate => candidate.id === describeProvider)
    const model = group?.models.find(candidate => candidate.id === describeModelId)
    return group !== undefined && model !== undefined ? `${group.name} / ${model.name}` : describeModelText
  })()
  const openPicker = (): void => {
    setPickerOpen(open => !open)
    if (catalog === null) {
      void remote.modelCatalog().then((result) => { if (result.ok) setCatalog(result.value) })
    }
  }
  ...
  const updateWorkflow = (index: number, patch: Partial<WorkflowDefinition>): void => {
  ...
  const updateParam = (wfIndex: number, paramIndex: number, fieldValue: JsonValue | undefined): void => {
    const workflow = workflows[wfIndex]
    if (workflow === undefined) return
    const param = workflow.nodeDefaults[paramIndex]
    if (param === undefined) return
  ...
  const doTestConnection = async (): Promise<void> => {
    ...
    else setStatus({ kind: 'error', text: `${t('testConnectionFail')}: ${result.error.message}` })
    setBusy(null)
  }
  ...
  const doFetch = async (): Promise<void> => {
    ...
    if (!result.ok) {
      setStatus({ kind: 'error', text: `${t('fetchedFail')}: ${result.error.message}` })
      setBusy(null)
      return
    }
  ...
  const fillDescription = async (addedIndex: number, workflow: WorkflowDefinition): Promise<void> => {
    const result = await remote.describeWorkflow({ workflow })
    if (!result.ok) return
    const latest = workflowsRef.current
    const target = latest[addedIndex]
    if (target === undefined || target.workflowId !== workflow.workflowId) return
    // Auto-fill never clobbers text the user already typed.
    const fillText = target.description === undefined || target.description === ''
    if (fillText && result.value.description === '' && result.value.attention === undefined) return
  ...
  const doValidate = async (index: number): Promise<void> => {
    ...
    setValidations(prev => ({ ...prev, [key]: result.ok ? result.value : { nodeInfoList: [] } }))
    setStatus(result.ok
      ? { kind: 'ok', text: t('validationOk') }
      : { kind: 'error', text: `${t('validationFailed')}: ${result.error.message}` })
    setBusy(null)
  }
  ...
  const doDescribe = async (index: number): Promise<void> => {
    ...
    setBusy(null)
    if (!result.ok) {
      setStatus({ kind: 'error', text: `${t('describeFailed')}: ${result.error.message}` })
      return
    }
    const current = workflows[index]
    if (current !== undefined) updateWorkflow(index, applyAnalysis(current, result.value, true))
  }
  ...
  const doRefresh = async (index: number): Promise<void> => {
    ...
    setStatus(null)
    const result = await remote.fetchWorkflow({ workflowId: workflow.workflowId })
    if (!result.ok) {
      setStatus({ kind: 'error', text: `${t('fetchedFail')}: ${result.error.message}` })
      setBusy(null)
      return
    ...
    setStatus({ kind: 'ok', text: t('fetchedOk', { nodes: data.nodes?.length ?? 0, params: data.nodeDefaults?.length ?? 0, media: data.mediaSlots?.length ?? 0 }) })
    setBusy(null)
  }
  ...
  const doRunTest = async (index: number): Promise<void> => {
    ...
    if (!result.ok) {
      setStatus({ kind: 'error', text: `${t('testRunFail')}: ${result.error.message}` })
      setBusy(null)
      return
    }
    setStatus({ kind: 'ok', text: t('testRunOk', { localId: result.value.localId, status: result.value.status }) })
    setBusy(null)
  }
  ...
  return (
    ...
    >
      <SecretField
        ...
        stateLabel={t(state.apiKeyConfigured ? 'apiKeyConfigured' : 'apiKeyUnset')}
      ...
      <ValueField
        ...
        numeric
        disabled={disabled}
        onEdit={(text) => { props.edit('runTimeoutMs', text) }}
        onReset={() => { props.resetField('runTimeoutMs') }}
      />
      <ValueField
        ...
        numeric
        disabled={disabled}
        onEdit={(text) => { props.edit('queueTimeoutMs', text) }}
        onReset={() => { props.resetField('queueTimeoutMs') }}
      />
      <ValueField
        ...
        numeric
        disabled={disabled}
        onEdit={(text) => { props.edit('maxConcurrentTasks', text) }}
        onReset={() => { props.resetField('maxConcurrentTasks') }}
      />
      <div className={css.field}>
        <div className={css.toggleRow}>
          ...
          <Switch
            ...
            disabled={disabled}
            onChange={(checked) => { props.editUploadUseLegacy(checked) }}
          />
        </div>
        ...
        <div className={css.toggleRow}>
          ...
          <div className={css.modelPicker} ref={pickerRef}>
            <button
              ...
              onClick={openPicker}
            >
              <span className={css.modelPickerValue}>{describeSelectionLabel}</span>
              <IconChevronDownOutline14 size={14} />
            </button>
            {pickerOpen
              ? (
                <div className={css.modelMenu} role="listbox" aria-label={t('describeModel')}>
                  <button
                    type="button"
                    role="option"
                    aria-selected={describeProvider === ''}
                    className={describeProvider === '' ? css.modelOptionActive : css.modelOption}
                    onClick={() => { editDescribeModel('', ''); setPickerOpen(false) }}
                  >
                    {catalog !== null
                      ? t('describeModelDefaultOf', { model: `${catalog.default.provider}/${catalog.default.model}` })
                    ...
                    <section key={group.id}>
                      ...
                      {group.models.map(model => (
                        <button
                          ...
                          role="option"
                          key={model.id}
                          aria-selected={describeProvider === group.id && describeModelId === model.id}
                          className={describeProvider === group.id && describeModelId === model.id ? css.modelOptionActive : css.modelOption}
                          onClick={() => { editDescribeModel(group.id, model.id); setPickerOpen(false) }}
                        >
                          {model.name}
                        </button>
                      ))}
                  ...
                  {catalog === null ? <p className={css.hint}>{t('describeModelLoading')}</p> : null}
                </div>
              )
              : null}
          </div>
      ...
      <div className={css.actions}>
        <Button size="sm" disabled={busy !== null} onClick={() => { void doTestConnection() }}>
          {busy === 'test' ? t('testing') : t('testConnection')}
        </Button>
        {status !== null
          ? (
            <p className={status.kind === 'error' ? css.statusError : css.statusOk} role="status">
              {status.text}
            </p>
          )
          : null}
      ...
      {workflows.length === 0 ? <p className={css.hint}>{t('noWorkflows')}</p> : null}
          ...
          <div key={key} className={css.workflow}>
            <div className={css.workflowHead}>
              <input
                ...
                aria-label={t('workflowLabel')}
                disabled={disabled}
                onChange={(e) => { updateWorkflow(wfIndex, { label: e.target.value }) }}
              />
              <a
              ...
              <button
                ...
                onClick={() => { void copyWorkflowId(key, workflow.workflowId) }}
              >
                {copiedKey === key ? <IconCheckOutline16 size={14} /> : <IconCopyOutline16 size={14} />}
              </button>
              <button
                type="button"
                className={css.paramsToggle}
                aria-expanded={expanded[key] === true}
                onClick={() => { setExpanded(prev => ({ ...prev, [key]: prev[key] !== true })) }}
              >
                <span className={expanded[key] === true ? css.chevronOpen : css.chevron}>
                  <IconChevronDownOutline14 size={14} />
                </span>
            ...
            <div className={css.descRow}>
              <textarea
                ...
                rows={2}
                disabled={disabled}
                onChange={(e) => { updateWorkflow(wfIndex, { description: e.target.value === '' ? undefined : e.target.value }) }}
              />
              <button
                type="button"
                className={css.iconButton}
                ...
                onClick={() => { void doDescribe(wfIndex) }}
              >
                <IconSparkle16 size={14} className={busy === `describe:${key}` ? css.spin : undefined} />
              </button>
            </div>
            ...
            {workflow.nodeDefaults.map((param, paramIndex) => (
              <div key={`${param.nodeId}.${param.fieldName}`} className={css.paramRow}>
                <button
                  type="button"
                  className={param.attention === true ? css.attentionActive : css.attention}
                  title={t('attentionHint')}
                  aria-label={t('attentionHint')}
                  aria-pressed={param.attention === true}
                  disabled={disabled}
                  onClick={() => { toggleAttention(wfIndex, paramIndex) }}
                >
                ...
                >
                  ...
                  {param.required ? <span className={css.requiredMark}> *</span> : null}
                </span>
                <span className={css.paramControl}>
                  <ParamInput
                    param={param}
                    disabled={disabled}
                    onChange={(next) => { updateParam(wfIndex, paramIndex, next) }}
                  />
                </span>
              </div>
            ))}
            ...
            {workflow.mediaSlots.length > 0
              ? (
                <ul className={css.mediaList}>
                  {workflow.mediaSlots.map(slot => (
                    <li key={`${slot.nodeId}.${slot.fieldName}`}>
            ...
            {validation !== undefined
              ? (
                <div>
                  {validation.missingParams !== undefined && validation.missingParams.length > 0
                    ? <p className={css.statusError}>{t('missingParams')}: {validation.missingParams.join(', ')}</p>
                    : null}
                  {validation.requiredMedia !== undefined && validation.requiredMedia.length > 0
                    ? <p className={css.validation}>{t('requiredMedia')}: {validation.requiredMedia.join(', ')}</p>
                    : null}
                  {validation.optionalEmpty !== undefined && validation.optionalEmpty.length > 0
                    ? <p className={css.validation}>{t('optionalEmpty')}: {validation.optionalEmpty.join(', ')}</p>
                    : null}
                  <details>
                    <summary className={css.validation}>{t('nodeInfoList')}</summary>
                    <pre className={css.payload}>{JSON.stringify(validation.nodeInfoList, null, 2)}</pre>
                  </details>
                </div>
              )
              : null}
            ...
            <div className={css.workflowActions}>
              <Button size="sm" disabled={busy !== null || disabled} onClick={() => { void doValidate(wfIndex) }}>
                {busy === `validate:${key}` ? t('validating') : t('validate')}
              </Button>
              <Button size="sm" disabled={busy !== null || disabled} onClick={() => { void doRunTest(wfIndex) }}>
                {busy === `test:${wfIndex}` ? t('testRunning') : t('testRun')}
              </Button>
              <Button size="sm" disabled={busy !== null || disabled} onClick={() => { void doRefresh(wfIndex) }}>
                {busy === `refresh:${wfIndex}` ? t('fetching') : t('refresh')}
              </Button>
              <Button size="sm" disabled={busy !== null || disabled} onClick={() => { doDelete(wfIndex) }}>
                {t('delete')}
              </Button>
            </div>
            </>)
...
function applyAnalysis(workflow: WorkflowDefinition, data: DescribeWorkflowData, fillText: boolean): WorkflowDefinition {
  ...
  if (data.attention !== undefined) {
    ...
    next.nodeDefaults = workflow.nodeDefaults.map((param) => {
      ...
      return marked.has(`${param.nodeId}.${param.fieldName}`) ? { ...rest, attention: true } : rest
    })
  }
  return next
}

function ParamInput(props: { param: NodeParamOverride; disabled: boolean; onChange: (value: JsonValue | undefined) => void }) {
  const { param, disabled, onChange } = props
  if (param.kind === 'select' && param.options !== undefined && param.options.length > 0) {
    return (
      <select
        className={css.input}
      ...
      >
        {param.fieldValue === undefined ? <option value="" disabled>—</option> : null}
  ...
  if (param.kind === 'number' || param.kind === 'int' || param.kind === 'float') {
    return (
      <NumberInput
  ...
  if (param.kind === 'json') {
    return (
      <input
        className={css.input}
        type="text"
        value={param.fieldValue === undefined ? '' : JSON.stringify(param.fieldValue)}
        disabled={disabled}
        onChange={(e) => {
          try { onChange(JSON.parse(e.target.value) as JsonValue) } catch { /* keep typing */ }
        }}
      />
    )
  }
...
function NumberInput(props: {
  ...
  const shown = draft ?? (typeof props.value === 'number' ? String(props.value) : '')
  return (
    <input
      className={css.input}
      type="text"
